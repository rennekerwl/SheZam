...
Use `tools/she_bop_fingerprint_tool.py` to generate `she_bop_fingerprints.csv` with the **same algorithm and parameters used at runtime** (`SimpleFingerprinter`: 16 kHz, frame size 1024, hop size 512, top 3 salient bins, peak salience >=45% of frame max, target-zone delta <=2 frames).
...
4. Produce a synthetic collision report (before vs after tuning):
   - `python tools/she_bop_fingerprint_tool.py collision-report --output-report tools/reports/she_bop_collision_report.md`
